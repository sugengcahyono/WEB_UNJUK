...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validasi nama produk (5-30 karakter)
    if (strlen($namaproduk) < 5 || strlen($namaproduk) > 100) {
        $response = array("status" => "error", "message" => "Nama produk harus terdiri dari 5-100 karakter");
    ...
    // Validasi deskripsi produk (tidak boleh kosong, maksimal 500 karakter)
    if (empty($deskripsiproduk) || strlen($deskripsiproduk) > 500) {
        $response = array("status" => "error", "message" => "Deskripsi produk tidak boleh kosong dan maksimal 500 karakter");
        echo json_encode($response);
        exit;
    }
